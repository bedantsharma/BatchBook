...
- [ ] **Deploy prerequisite: run `uv run alembic upgrade head` against the production `DATABASE_URL`** before the endpoints can serve real traffic — the hand-written migration was never applied against a live DB in the dev environment due to no DB connectivity
