...
async def test_get_owner_stats_with_enrollments_and_fees(client, db_session):
    ...
    batch = BatchSchema(
        id=200, institute_id=institute_id, name="Class 10 Maths",
    ...
    rows = await db_session.execute(sa_select(EnrollmentSchema).where(EnrollmentSchema.batch_id == batch.id))
...
async def test_get_owner_stats_with_attendance(client, db_session):
    ...
    import datetime as _dt
    batch = BatchSchema(
        id=300, institute_id=institute_id, name="Physics",
